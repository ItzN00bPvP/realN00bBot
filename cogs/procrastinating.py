...
class procrastinating(commands.Cog):
    ...
    @cog_ext.cog_slash(guild_ids=config.slash_procrastinating, name="procrastinating",
                       description="Makes a procrastinating domain for a User.", options=[
            create_option(
                name="name",
    ...
    async def _bald(self, ctx: SlashContext, name: str, verb: str = "is"):
        ...
        req = requests.post(
            f"https://api.cloudflare.com/client/v4/zones/{cloudflare_procrastinatingathomezoneid}/dns_records",
